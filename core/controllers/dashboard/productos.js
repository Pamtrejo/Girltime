$(document).ready(function()
{
    ...
    showSelectMarca('create_marca', null);
})
    ...
    rows.forEach(function(row){
        ...
        content += `
            <tr>
                ...
                <td>
                    <a href="#" onclick="modalUpdate(${row.id_producto})" class="pink-text tooltipped" data-tooltip="Modificar"><i class="material-icons">mode_edit</i></a>
                    <a href="#" onclick="confirmDelete(${row.id_producto}, '${row.imagen_producto}')" class="black-text tooltipped" data-tooltip="Eliminar"><i class="material-icons">delete</i></a>
                </td>
            </tr>
        `;
    });
...
//Función para cargar las marcas en el select del formulario
function showSelectMarca(idSelect, value)
{
    $.ajax({
        url: apiProductos + 'readMarcas',
        type: 'post',
        data: null,
        datatype: 'json'
    })
    .done(function(response){
        ...
        if (isJSONString(response)) {
            ...
            if (result.status) {
                let content = '';
                if (!value) {
                    content += '<option value="" disabled selected>Seleccione una marca</option>';
                }
                result.dataset.forEach(function(row){
                    if (row.id_marca != value) {
                        content += `<option value="${row.id_marca}">${row.marca}</option>`;
                    } else {
                        content += `<option value="${row.id_marca}" selected>${row.marca}</option>`;
                    }
                });
                $('#' + idSelect).html(content);
            } else {
